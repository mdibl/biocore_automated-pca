...
parent_folder = file.path(json$folders$output_folder)
...
```{r}
path_2_design_file = paste0(json$folders$data_folder,json$input_files$design_file)
path_2_counts_file = paste0(json$folders$data_folder,json$input_files$count_file)
print(paste("Design file: ", path_2_design_file))
print(paste("Estimated counts file: ", path_2_counts_file))
```

```{r}
design = read.table(file.path(path_2_design_file), header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
counts = read.table(file.path(path_2_counts_file), header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
...
if (file.exists(as.character(json_copy$path_2_results$normalized_rld))){
  ...
  norm_1 = read.table(path2_matrix_norm_1, header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
...
#facetplots added 7/18/21
PC1_facetplot= json_copy$figures$PC1_facetplot
PC2_facetplot= json_copy$figures$PC2_facetplot
PC3_facetplot= json_copy$figures$PC3_facetplot
```
...
The facet plots demonstrate the sample loadings along the first three principal components with respect to design features to possibly suggest what is driving these top principal components. If two design variables are entered, the samples are separated into facets by the first design variable. Additionally, follow a visualization of PC1 vs. PC2 coordinates and PC2 vs. PC3. Note: there might be more than 3 principal components. Here I display the first three. To get the coordinates of all the found PCs, refer to the PC object found at:
...
#added 7/18/21
knitr::include_graphics(PC1_facetplot)
knitr::include_graphics(PC2_facetplot)
knitr::include_graphics(PC3_facetplot)
```
...
design_new = read.table(file.path(path_design_new), header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
kable(design_new)
```
...
regression_pc_eigen = read.table(file.path(path_2_pc_eigen), header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
...
for (j in 1:length(all_tables)){
  result_table = read.table(file.path(all_tables[j]), header = TRUE, sep = "\t", row.names = 1, check.names=FALSE)
